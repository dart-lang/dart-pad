...
class Console {
  ...
  void showOutput(String message, {bool error = false}) {
    ...
    if( _ansiConsoleHandler.ansiStylesActive || ansiSeqPos != -1 ) {
      // we either have ansi styles active or there is a possible ansi escape code in this message...
...
class AnsiConsoleHandler {
  ...
  /// We set this to [darkModeAnsiColors] or [lightModeAnsiColors] in constructor depending on [_darkMode]
  late final List<int> _themeModeAnsiColors;

  // Certain ranges that are matched here do not contain real graphics rendition sequences. For
  // the sake of having a simpler expression, they have been included anyway.
  static final RegExp _ansiMatcher = RegExp(r'^(?:[34][0-8]|9[0-7]|10[0-7]|[0-9]|2[1-5,7-9]|[34]9|5[8,9]|1[0-9])(?:;[349][0-7]|10[0-7]|[013]|[245]|[34]9)?(?:;[012]?[0-9]?[0-9])*;?m$');

  /// List of currently active Ansi style class names we need will apply to outgoing text
  ...
  AnsiConsoleHandler( bool darkMode ) {
    // Use theme colors from [darkModeAnsiColors] or [lightModeAnsiColors] depending on
    // the setting of [darkMode]
    _themeModeAnsiColors = darkMode ? darkModeAnsiColors : lightModeAnsiColors;
  }

  /// return a 0x00RRGGBB style color int using passed [red],[green] and [blue] values
  ...
  void appendStylizedStringToContainer(DivElement div, String buffer, 
                      List<String>appendAnsiStyleClassNames,
  ...
  /// Set the member variable corresponding to [colorType] to the css 'rgb(...)' color
  /// string computed from [rgbcolor] (or specified by [colorAsString]).
  ...
  void changeSpecifiedCustomColor( String colorType, int? rgbcolor, [String? colorAsString]) {
  ...
  /// [_colorsInverted] flag to make sure it is appropriate to turn ON or OFF (if it is already inverted don't call
  void reverseForegroundAndBackgroundColors() {
    final String? oldFgColor = _customFgColor;
    changeSpecifiedCustomColor('foreground', null, _customBgColor);  // we have strings already, so pass those
    changeSpecifiedCustomColor('background', null, oldFgColor);  
  }

  /// Calculate the color for [ansiColorNumber] from the color set defined in the ANSI 8-bit standard.
  /// [ansiColorNumber] should be a number ranging from 16 to 255 otherwise it is considered an invalid
  /// color and `null` will be returned.
  ...
  /// The [styleCodes] contains the list of integer codes that make up the full ANSI
  /// sequence, including the two defining codes and the three RGB codes.
  /// The [colorType] parameter must be `'foreground'`, `'background'` or `'underline'`
  /// corresponding to which of our custom colors we are setting
  ...
  void set24BitAnsiColor(List<int> styleCodes, String colorType) {
    if (styleCodes.length >= 5 &&
      ...
      changeSpecifiedCustomColor(colorType, rgba(styleCodes[2], styleCodes[3], styleCodes[4]));
    }
  }
  ...
  /// [30-37, 90-97, 40-47, 100-107]. If not on one of these ranges it will be ignored
  /// See https://en.wikipedia.org/wiki/ANSI_escape_code#3-bit_and_4-bit for more info
  void setBasicAnsiColor(int styleCode) {
    String? colorType;
    int? colorIndex;
    ...
    if (colorIndex != null && colorType!=null) {
      ...
      final int? color = (colorIndex>=0 && colorIndex<=_themeModeAnsiColors.length) ? 
                                   _themeModeAnsiColors[colorIndex] : null;
      if (color!=null) {
  ...
  void set8BitAnsiColor(List<int> styleCodes, String colorType ) {
    ...
    if (colorNumber >= 0 && colorNumber <= 15) {
      ...
      // depending on if colorType is 'foreground' or 'background'
      colorNumber += 30;
      if (colorNumber >= 38) {
        // Bright colors
        colorNumber += 52;
  ...
  /// codes (reset, bold, italic, underline, double-underline, overline, strikethrough, 
  /// superscripts, subscripts, dim, blink, rapid-blink, invert, hidden, and fonts 1-10)
  /// We also handle normal foreground and background colors, and bright foreground and
  /// background colors codes here.  Not to be used for codes containing advanced colors.
    ...
    for(final int code in styleCodes) {
      switch (code) {
        case 0: // reset
          ...
          break;
        case 1: // bold
          if(!ansiStyleClassNames.contains('ansi-bold')) ansiStyleClassNames.add('ansi-bold');
          break;
        case 2: // dim
          if(!ansiStyleClassNames.contains('ansi-dim')) ansiStyleClassNames.add('ansi-dim');
          break;
        case 3: // italic
          if(!ansiStyleClassNames.contains('ansi-italic')) ansiStyleClassNames.add('ansi-italic');
          break;
        case 4: // underline
          if(!ansiStyleClassNames.contains('ansi-underline')) ansiStyleClassNames.add('ansi-underline');
          break;
        case 5: //blink
          if(!ansiStyleClassNames.contains('ansi-blink')) ansiStyleClassNames.add('ansi-blink');
          break;
        case 6: // rapid blink
          if(!ansiStyleClassNames.contains('ansi-rapid-blink')) ansiStyleClassNames.add('ansi-rapid-blink');
          break;
        case 7: // invert foreground and background
          // we must track if we have inverted colors, subsequent calls are ingored until cleared with 27
          if (!_colorsInverted) {  
        ...
        case 8: // hidden (but selectable/copy-able)
          if(!ansiStyleClassNames.contains('ansi-hidden')) ansiStyleClassNames.add('ansi-hidden');
          break;
        case 9: // crossed-out
          if(!ansiStyleClassNames.contains('ansi-strike-through')) ansiStyleClassNames.add('ansi-strike-through');
          break;
        case 10: // normal default font
          ansiStyleClassNames.removeWhere( (style) => style.startsWith('ansi-font') );
          break;
        ...
        case 21: // make double underline
          ansiStyleClassNames.removeWhere( (style) => (style=='ansi-underline' || style=='ansi-double-underline') );
          ansiStyleClassNames.add('ansi-double-underline');
          break;
        case 22: // bold and dim Off
          ansiStyleClassNames.removeWhere( (style) => (style=='ansi-bold' || style=='ansi-dim') );
          break;
        case 23: // Neither italic or blackletter (font 10)
          ansiStyleClassNames.removeWhere( (style) => (style=='ansi-italic' || style=='ansi-font-10') );
          break;
        case 24: // underline off (Neither singly nor doubly underlined)
          ansiStyleClassNames.removeWhere( (style) => (style=='ansi-underline' || style=='ansi-double-underline') );
          break;
        case 25: // not blinking
          ansiStyleClassNames.removeWhere( (style) => (style=='ansi-blink' || style=='ansi-rapid-blink') );
          break;
        case 27: // if we are currently invered then reversed/clear the inversion, otherwise ignored
          if (_colorsInverted) {
        ...
        case 29: // not crossed-out
          ...
          break;
        case 53: // overline
          if(!ansiStyleClassNames.contains('ansi-overline')) ansiStyleClassNames.add('ansi-overline');
          break;
        ...
        case 59: // default underline color
          ...
          break;
        case 73: // superscript
          ansiStyleClassNames.removeWhere( (style) => (style=='ansi-superscript' || style=='ansi-subscript'));
  ...
  /// This handler is called with a [message] and possibly the position of the first found ANSI
  /// escape sequence (ESC+OPENBRACKET) contained in [posOfFirstFoundESCBracket] (or -1
  /// if no sequence was found in [message]).  
  /// If [posOfFirstFoundESCBracket] is -1 (if no escape sequence was found) then
  /// we short circuit and set currentPos to the LENGTH of [message] so we immediately
  /// copy the entire message, style and output it, bypassing searching the string for ESC sequences.
  /// (This happpens when [ansiStylesActive] is true so we have active ansi styles 
  /// to be applied to this [message])
  DivElement handleAnsiOutput(final String message, final int posOfFirstFoundESCBracket) {
    String buffer = '';
    final int textLength = message.length;
    int currentPos = (posOfFirstFoundESCBracket!=-1) ? posOfFirstFoundESCBracket : textLength;

    final div = DivElement();

    // we may have come in with the location of the first ESC[ sequence, so copy the first
    ...
    while(currentPos<textLength) {
      ...
      if (message.codeUnitAt(currentPos) == 27 && message[currentPos + 1] == '[') {
        ...
        if( sequenceFound ) {
          ...
          appendStylizedStringToContainer( div, buffer, ansiStyleClassNames, _customFgColor, _customBgColor, _customUnderlineColor );
          buffer = '';

          if (_ansiMatcher.hasMatch(ansiSequence)) {
            final List<int> styleCodes = ansiSequence.substring(0, ansiSequence.length - 1) // Remove final 'm' character.
    ...
    ansiStylesActive = (ansiStyleClassNames.isNotEmpty || _customFgColor!=null ||
                           _customBgColor!=null || _customUnderlineColor!=null);  
