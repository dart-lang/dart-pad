...
function generateCode(base: string, count: number): string {
  ...
  for (let i = 0; i < count; i++) {
    res +=
      base.replace(/greet/g, "greet" + i).replace(/Foo/g, "Foo" + i) + "\n";
  }
  return res;
}
...
window.runBenchmark = (dartCallback: (code: string) => Int32Array) => {
  ...
  document.body.appendChild(
    document.createTextNode("Incremental (Modify at END) 10k lines"),
  );
  ...
  runPerf("Incremental Parse JS (End)", () => {
    ...
    jsParser.parse(jsInputModEnd10k, shifted, [
      { from: 0, to: jsModEnd10k.length },
    ]);
  });
  ...
  let dartChangesEnd = [
    {
      fromA: dart10k.length,
      toA: dart10k.length,
      fromB: dart10k.length,
      toB: dart10k.length + 1,
    },
  ];
  runPerf("Incremental Parse Dart SDK (End)", () => {
    ...
    dartParser.parse(dartInputModEnd10k, shifted, [
      { from: 0, to: dartModEnd10k.length },
    ]);
  });

  document.body.appendChild(document.createElement("hr"));
  document.body.appendChild(
    document.createTextNode("Incremental (Modify at START) 10k lines"),
  );
  ...
  runPerf("Incremental Parse JS (Start)", () => {
    ...
    jsParser.parse(jsInputModStart10k, shifted, [
      { from: 0, to: jsModStart10k.length },
    ]);
  });
  ...
  runPerf("Incremental Parse Dart SDK (Start)", () => {
    ...
    dartParser.parse(dartInputModStart10k, shifted, [
      { from: 0, to: dartModStart10k.length },
    ]);
  });
  ...
  document.body.appendChild(
    document.createTextNode(
      "Incremental (Unterminated Comment at START) 10k lines",
    ),
  );
  ...
  runPerf("Incremental Parse JS (Unterminated Comment)", () => {
    let oldFragments = TreeFragment.addTree(jsTree10k);
    let shifted = TreeFragment.applyChanges(
      oldFragments,
      jsChangesUnterminated,
    );
    jsParser.parse(jsInputModUnterminated10k, shifted, [
      { from: 0, to: jsModUnterminated10k.length },
    ]);
  });
  ...
  runPerf("Incremental Parse Dart SDK (Unterminated Comment)", () => {
    ...
    let shifted = TreeFragment.applyChanges(
      oldFragments,
      dartChangesUnterminated,
    );
    dartParser.parse(dartInputModUnterminated10k, shifted, [
      { from: 0, to: dartModUnterminated10k.length },
    ]);
  });
  ...
  document.body.appendChild(
    document.createTextNode(
      "Incremental (Unbalanced Bracket at START) 10k lines",
    ),
  );
  ...
  runPerf("Incremental Parse JS (Unbalanced Bracket)", () => {
    ...
    jsParser.parse(jsInputModUnbalanced10k, shifted, [
      { from: 0, to: jsModUnbalanced10k.length },
    ]);
  });
  ...
  runPerf("Incremental Parse Dart SDK (Unbalanced Bracket)", () => {
    ...
    let shifted = TreeFragment.applyChanges(
      oldFragments,
      dartChangesUnbalanced,
    );
    dartParser.parse(dartInputModUnbalanced10k, shifted, [
      { from: 0, to: dartModUnbalanced10k.length },
    ]);
  });
  ...
  document.body.appendChild(
    document.createTextNode(
      "Incremental (Insert Template lines at MIDDLE) 10k lines",
    ),
  );

  let jsMiddleIdx = js10k.indexOf("function greet500()");
  let jsModMiddle10k =
    js10k.slice(0, jsMiddleIdx) + js10Lines + js10k.slice(jsMiddleIdx);
