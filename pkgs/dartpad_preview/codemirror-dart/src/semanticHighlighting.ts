  ...
  const plugin = ViewPlugin.fromClass(
    class {
      ...
      update(update: ViewUpdate) {
        if (
          ...
          update.transactions.some((tr) =>
            tr.effects.some((e) => e.is(forceSemanticTokensRefresh)),
          )
        ) {
          ...
          this.updateId = window.setTimeout(
            () => this.fetchSemanticTokens(update.view, 1),
            250,
          );
        }
      }
      ...
      async fetchSemanticTokens(view: EditorView, retries = 5) {
        ...
        if (!this.isCurrent(requestGeneration, view, doc)) return;
        ...
        try {
          ...
          for (let i = 0; i < data.length; i += 5) {
            ...
            if (tag) {
              ...
              if (themeClass) {
                if (currentLine >= doc.lines) continue;
                const line = doc.line(currentLine + 1);
                const start = line.from + currentCol;
                if (start < line.from || start + length > line.to) continue;
                builder.add(
                  start,
                  start + length,
                  Decoration.mark({ class: themeClass + " cm-semantic-token" }),
                );
              }
            }
          }
      ...
      isCurrent(requestGeneration: number, view: EditorView, doc: Text) {
        return (
          !this.destroyed &&
