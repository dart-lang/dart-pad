...
void main() {
  ...
  test(
    'SDK replacement retains empty root, initial metadata, edited files, current entrypoint and external tabs',
    () async {
      final contents = testProjectContents({
        'README.md': '# Project',
        'pubspec.yaml': 'name: demo',
      ...
      final api = _Workspace();
      await ProjectLoader.writeFiles(api.root, contents);
      final externalUri = Uri.parse('file:///pub-cache/demo/main.dart');
      _WorkspaceRepository repository(WorkspaceResourceApi api, {required bool dart}) => _WorkspaceRepository(
        ...
        workspaceFuture: Completer<Workspace>().future,
        externalFiles: {externalUri: 'void main() {}'},
      );
      ...
      await old.tabs.openExternalFile(externalUri);
      ...
      try {
        ...
        expect(next.tabs.activeFile, externalUri.toString());
        expect(next.tabs.openTabs.map((tab) => tab.path), ['README.md', externalUri.toString()]);
      } finally {
