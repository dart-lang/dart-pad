  ...
  for (const file of fs.readdirSync(casesDir)) {
    ...
    describe(groupName, () => {
      for (const t of testCases) {
        ...
        let expectedTree = lines.slice(separatorIdx + 1).join('\n').trim();
        ...
        it(`verifies tree format for ${name}`, () => {
          ...
          assert.strictEqual(printTree(tree), expectedTree, "Tree formatting must match standard syntax representations natively");
        });
      }
    });
  }
