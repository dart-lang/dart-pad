...
describe("Dart Analyzer Parsing", () => {
  it("parses basic code correctly into TopLevel blocks", () => {
    ...
    assert.strictEqual(printTree(tree), "Program(Program(TopLevel(Keyword, Identifier, ArgumentList(Punctuation, Punctuation), Block(Punctuation, Identifier, ArgumentList(Punctuation, String, Punctuation), Punctuation, Punctuation)), TopLevel(Identifier, Identifier, Operator, Number, Punctuation)))");
  });
  ...
  it("handles string types natively", () => {
    ...
    assert.strictEqual(printTree(tree), "Program(Program(TopLevel(Identifier, Identifier, Operator, String, Punctuation)))");
  });
});
