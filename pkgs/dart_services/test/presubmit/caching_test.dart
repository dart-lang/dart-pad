...
void defineTests(bool hasRedis) {
  ...
  group('RedisCache', () {
    ...
    Future<Process> startRedisProcessAndDrainIO(int port) async {
      final newRedisProcess = await Process.start('redis-server', [
        '--port',
        port.toString(),
      ]);
      unawaited(
        singleStreamOnly.synchronized(() async {
    ...
    test('Verify disconnected cache logs errors and returns nulls', () async {
      await singleTestOnly.synchronized(() async {
        logMessages = [];
        final redisCacheBroken = RedisCache(
          'redis://localhost:9502',
          sdk,
          'cversion',
        );
        try {
    ...
    test(
      'Verify cache that starts out disconnected retries and works (slow)',
      () async {
        await singleTestOnly.synchronized(() async {
          logMessages = [];
          final redisCacheRepairable = RedisCache(
    ...
    test(
      ...
      () async {
        await singleTestOnly.synchronized(() async {
          ...
          await expectLater(
            await redisCache.get('beforeStop'),
            equals('truth'),
          );
          expect(
            logMessages.join('\n'),
