...
final log = DartPadLogger('caching');
...
class RedisCache implements ServerCache {
  ...
  @override
  Future<void> shutdown() {
    log.genericInfo('$_logPrefix: shutting down...');
    _isShutdown = true;
    _connection?.close();
    return disconnected;
  }
  ...
  void _reconnect([int retryTimeoutMs = _connectionRetryBaseMs]) {
    ...
    log.genericInfo('$_logPrefix: reconnecting to $redisUri...');
    ...
    (redisUri.hasPort
        ...
        .then((newConnection) {
          log.genericInfo('$_logPrefix: Connected to redis server');
